...
class DownloadLimiter:
    ...
    def __init__(self, config_loader: ConfigLoader | None = None, config_path: str | None = None):
    ...
    def can_download(self, user_id: int) -> bool:
        ...
        if not can_proceed:
            logger.info(
                f"User {user_id} has reached download limit ({active_count}/{self.MAX_DOWNLOADS_PER_USER})"
            )
